...
import flywheel.analytics.oauth_manager
import requests
...
def mocked_requests_get(*args, **kwargs):
    ...
    if kwargs['url'] == 'http://127.0.0.1:80/token':
        ...
        if mocked_requests_get.error_count < 0 or mocked_requests_get.error_count > 0:
            ...
            mocked_requests_get.error_count = -1
            return MockResponse({"json_data" : {"access_token": "test_token", "expires_in": 4000}}, 200)
    elif kwargs['url'] == 'https://api.theflywheel.app/event-capture':
...
class TestOauthManager(unittest.TestCase):
    ...
    def test_oauth_success(self, mock_post):
        manager = flywheel.analytics.oauth_manager.OauthManager("id", privatekey, "keyid", "http://127.0.0.1:80")
    ...
    def test_oauth_fail_unrecoverably(self, mock_post):
        manager = flywheel.analytics.oauth_manager.OauthManager("id", privatekey, "keyid", "http://127.0.0.1:400")
    ...
    def test_oauth_fail_with_retries(self, mock_post):
        manager = flywheel.analytics.oauth_manager.OauthManager("id", privatekey, "keyid", "http://127.0.0.1:500")
        with self.assertRaises(Exception) as context:
            manager.get_token()
        self.assertTrue(manager.thread.is_alive)
    ...
    def test_oauth_rate_limit_delay(self, mock_sleep, mock_post):
        manager = flywheel.analytics.oauth_manager.OauthManager("id", privatekey, "keyid", "http://127.0.0.1:429")
